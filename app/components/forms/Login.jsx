...
export default function Login() {
  ...
  const handleSubmit = (e) => {
    ...
    toast.promise(
      postLogin.mutateAsync(formData).then(async (data) => {
  ...
  return (
    <div>
      <h5
        className={`text-[24px] sm:text-[26px] text-primaryblack flex flex-col ${
      ...
      <p
        className={`text-[16px] sm:text-[18px] text-ash mt-[30px] flex flex-col ${
          lang === "en" ? "flex-row" : "flex-row-reverse text-right"
        }`}
      >
      ...
      <div className="mt-[50px]">
        {/* tab buttons  */}
        <div
          className={`flex gap-[30px] pb-[10px] ${
        ...
        <div className="mt-[25px] sm:mt-[50px]">
          {option === 1 ? (
            ...
            <form onSubmit={handleSubmit}>
              ...
              <button
                type="submit"
                className={`py-[12px] text-[14px] sm:text-[16px] ${
                  option === 2 ? "bg-primary text-white" : "bg-creamline"
                } rounded-xl mt-[40px] w-full ${
                  loading ? "cursor-not-allowed bg-creamline" : "cursor-pointer"
                }`}
                disabled={postLogin.isLoading || loading}
              >
                {!loading ? (lang === "en" ? "Sign in" : "تسجيل الدخول") : null}

                {loading && (
                  <LoadingSvg label="Signing in" color="text-white" />
                )}
              </button>
            </form>
          )}
          ...
          <p
            className={`text-primaryblack mt-[20px] sm:mt-[40px] flex flex-col ${
              lang === "en" ? "flex-row" : "flex-row-reverse"
            }`}
          >
            {t("dont_have_account")}{" "}
