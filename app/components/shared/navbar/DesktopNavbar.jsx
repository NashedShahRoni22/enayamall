...
export default function DesktopNavbar({
  ...
  return (
    <>
      <div
        className={`hidden lg:block relative ${lang === "ar" ? "rtl" : "ltr"}`}
      >
        <Container>
          {/* Top section */}
          <div
          ...
          >
            ...
            <div className="flex-1 w-full mx-8">
              <div
                className={`flex-1 flex items-center gap-3 bg-gray-50 rounded-full px-4 py-3 focus-within:bg-white focus-within:ring-2 focus-within:ring-primary/20 border border-transparent focus-within:border-primary/30 transition-all ${
                  lang === "ar" ? "flex-row-reverse" : ""
                }`}
              >
                <input
            ...
            >
              ...
              <Link href="/wishlist">
                <button className="cursor-pointer bg-primary text-white size-10 rounded-full flex justify-center items-center hover:bg-primary/90 transition-colors relative">
                  ...
                  {wishlistCount > 0 && (
                    <>
                      <span className="absolute -top-2 -right-2 bg-brand-pink animate-ping text-white text-xs rounded-full size-5 flex items-center justify-center"></span>
                ...
                {isLoggedIn && showAccountPopover && (
                  <div
                    className={`absolute top-9 ${
                      lang === "ar" ? "-left-9" : "-right-9"
                    } mt-2 bg-white rounded-lg shadow-lg border border-gray-200 py-2 min-w-[180px] z-50`}
                  >
                    <Link
                      href="/account"
                      className={`w-full px-4 py-3 text-sm transition-colors flex items-center gap-3 text-gray-700 cursor-pointer ${
                        lang === "ar"
                    ...
                    <Link
                      href="/orders"
                      className={`w-full px-4 py-3 text-sm transition-colors flex items-center gap-3 text-gray-700 ${
                        lang === "ar"
                          ? "flex-row-reverse text-right"
                          : "text-left"
                      }`}
                    ...
                    <button
                      ...
                      className={`cursor-pointer w-full px-4 py-3 text-sm transition-colors flex items-center gap-3 text-red-600 ${
                        lang === "ar"
                          ? "flex-row-reverse text-right"
                          : "text-left"
                      }`}
          ...
          <Container>
            ...
            >
              ...
              >
                ...
                >
                  <button
                    className={`flex gap-2 items-center cursor-pointer w-[200px] transition-colors ${
                      lang === "ar" ? "flex-row-reverse" : ""
                    }`}
                  >
              ...
              >
                {contactInfo?.phone && (
                  <div
                    className={`flex gap-2 items-center border border-white/30 py-2 px-4 rounded-full hover:bg-white/10 transition-colors ${
                      lang === "ar" ? "flex-row-reverse" : ""
                    }`}
                  >
                    <Phone className="size-4" />
                    <p className="text-sm">
                      {t.hotline} {contactInfo.phone}
                    </p>
                  </div>
                )}
                ...
                <div className="relative" ref={languageRef}>
                  ...
                  {showLanguageDropdown && (
                    <div
                      ...
                      } mt-2 bg-white rounded-lg shadow-lg border border-gray-200 py-2 min-w-[120px] z-50`}
                    >
                      {languages.map((language) => (
                        <button
                          key={language.code}
                          onClick={() => handleLanguageChange(language.code)}
                          className={`w-full px-4 py-2 text-sm transition-colors cursor-pointer flex items-center gap-2 ${
                            lang === language.code
