        ...
        <div className="lg:max-w-7xl lg:mx-auto">
          <div className='py-[60px] lg:py-[120px]'>
            ...
            <div className="sticky top-18 lg:static lg:flex lg:gap-[24px] bg-white z-10">
              {/* for large device  */}
              <div className="hidden  lg:w-2/6 2xl:w-1/4 lg:flex gap-[12px] items-center">
                <Image src={filterIcon} alt="filter icon" />
                <p className="text-[24px] text-button">Filtered by</p>
              </div>
                  ...
                  <div className="flex gap-[20px] items-center">
                    <div className="flex gap-[8px]">
                      <button
                        ...
                        className={`cursor-pointer p-[16px] rounded-[10px] ${viewStyle === 0 ? "bg-natural text-white" : "bg-white text-natural"}`}>
                        {/* <Image src={gridIcon} alt="filter icon" /> */}
                        <IoGridOutline className="text-[20px]" />
                      </button>
                      <button
                        onClick={() => setViewStyle(1)}
                        className={`cursor-pointer p-[16px] rounded-[10px] ${viewStyle === 1 ? "bg-natural text-white" : "bg-white text-natural"}`}>
                        {/* <Image src={listIcon} alt="list icon" /> */}
                        <FaList className="text-[20px]" />
                      </button>
                      <button
                        onClick={() => setShowFilters(!showFilters)}
                        className={`lg:hidden cursor-pointer p-[16px] rounded-[10px] ${showFilters ? "bg-natural text-white" : "bg-white text-natural"}`}>
                        {/* <Image src={listIcon} alt="list icon" /> */}
                      ...
                      <div className="flex flex-col items-start absolute min-w-full z-10 p-[4px] top-11 lg:top-14 left-0 bg-white rounded-[10px] border border-creamline">
                        {sortOptions.map((label, index) => (
                          <button
                            ...
                            className={`px-[12px] py-[8px] min-w-full text-left text-[14px] lg:text-[16px] rounded-[10px] hover:text-natural hover:bg-creamline cursor-pointer ${sortOption === index && 'font-[550] text-natural'}`}
                          >
                            {label}
                          </button>
                        ))}
                ...
                {/* pagination here  */}
                {
                  ...
                  <div className="mt-20 flex flex-col gap-5 items-center justify-center">
                    ...
                    <button
                      ...
                      className="px-8 py-3 bg-natural text-white text-sm rounded-[5px] cursor-pointer hover:bg-creamline hover:text-primarymagenta disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {
                        isLoadingMore ? (
          ...
          <div className="lg:hidden fixed top-0 left-0 z-[9999] bg-black/40 min-w-full h-full flex">
            ...
            <div className="bg-white w-5/6 sm:w-1/2 h-screen flex flex-col gap-[20px] py-[30px] px-5 overflow-y-scroll">
              <div className="flex justify-between items-center">
                <button onClick={() => setShowFilters(false)} className="flex gap-[12px] items-center">
                  <Image src={filterIcon} alt="filter icon" />
                  <p className="text-[20px] text-button">Filtered by</p>
                </button>

                <button onClick={() => setShowFilters(false)} className="p-[8px] bg-creamline rounded-full">
                  <IoCloseOutline className="text-[24px] text-natural" />
                </button>
              </div>
